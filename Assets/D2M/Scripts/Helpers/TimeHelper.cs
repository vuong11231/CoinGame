...
public static class TimeHelper
{
    ...
    public static string ConverSecondtoMiLiSecond(float second)
    {
        string answer = "";

        int mili = (int)(second * 100) % 100;

        TimeSpan t = TimeSpan.FromSeconds(second);

        long  a = (int)second / 60;

        if (a >= 60)
        {
            answer = string.Format("{0:D1}h{1:D2}p{2:D2}s",
            t.Hours,
            t.Minutes,
            t.Seconds
            );
        ...
        {
            answer = string.Format("{0:D1}p{1:D2}s",
            t.Minutes,
            t.Seconds);
